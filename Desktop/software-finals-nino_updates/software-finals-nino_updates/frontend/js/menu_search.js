...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    const data = await response.json();
    if (data.success) {
      // Use the database products instead of the hardcoded menuData
      renderCards(data.products); 
    }
  } catch (err) {
    renderCards(menuData); // Fallback to hardcoded if server is down
  }
  syncCartFromDB(); // Get the user's saved items
});